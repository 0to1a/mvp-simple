...
func TestAuthRequired(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Set up authorization header based on test case
			switch tt.name {
			case "valid token":
				...
				tt.expectedBody = `{"message":"authorized","user_id":123}`
			case "expired token":
			...
			if tt.expectedStatus == http.StatusOK {
				// For successful cases, check that response contains expected structure
				...
				// For error cases, check exact error message
				if !contains(responseBody, tt.expectedBody) {
					t.Errorf("Expected response to contain %q, got: %s", tt.expectedBody, responseBody)
				}
			}
		})
	}
